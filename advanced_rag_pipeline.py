...
개선 사항:
  ...
  2. 대화 기록 자동 관리            — ConversationBufferWindowMemory + 슬라이딩 윈도우
...
class AdvancedRAGPipeline:
    ...
    def _retrieve(self, query: str) -> List[Document]:
        ...
        if _VECTOR_STORE_AVAILABLE and isinstance(self.vectorstore, AdvancedVectorStore):
            ...
            return [r.document for r in report.results]
    ...
    def ask(
        ...
        Returns:
            RAGResult — answer, citations, elapsed_sec 등 포함
        """
        ...
        logger.info("답변 생성 완료 — 유형: %s, 출처: %d개, %.3fs",
                    qtype, len(docs), elapsed)
        return result
...
if __name__ == "__main__":
    ...
    with AdvancedRAGPipeline(
        vectorstore=store,
        llm_model="gpt-4",
        retrieval_strategy="hybrid",   # "similarity" | "mmr" | "hybrid"
        domain="legal",
        k=5,
    ) as rag:

        # 단일 질문 (유형 자동 감지)
        result = rag.ask("이 계약서의 주요 조건들을 비교 분석해주세요.")
        result.print_report()

        # 다중 쿼리 검색 (더 넓은 범위)
        result2 = rag.ask("손해배상 조항은?", use_multi_query=True)
        ...
        # 대화형 — 이전 맥락 자동 유지
        rag.ask("방금 말한 조항에서 예외 사항이 있나요?", use_history=True)

        # LangChain 체인 객체로 사용
        chain    = rag.build_adaptive_chain()
        response = chain.invoke({"question": "계약 당사자는 누구인가요?"})
        print(f"\n▶ 체인 직접 호출: {response[:200]}")
